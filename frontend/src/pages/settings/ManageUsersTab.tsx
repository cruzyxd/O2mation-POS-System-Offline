...
import { Eye, EyeOff, Plus, Save, Trash2, Power, UserPlus, Shield, User as UserIcon, X, AlertTriangle, Pencil } from "lucide-react";
...
export function ManageUsersTab() {
  ...
  async function loadUsers() {
    ...
    try {
      const usersList = await usersService.listUsers(accessToken);
      setUsers(usersList);
    } catch (err) {
  ...
  async function handleCreateUser(e: React.FormEvent) {
    ...
    try {
      ...
      const newUser = await usersService.createUser(accessToken, request);
  ...
  async function handleUpdateUser(userId: number) {
    ...
    try {
      ...
      const updatedUser = await usersService.updateUser(accessToken, userId, request);
  ...
  async function handleToggleActive(userId: number, isActive: boolean) {
    ...
    try {
      await usersService.toggleUserActive(accessToken, userId, { is_active: !isActive });
  ...
  async function handleDeleteUser(userId: number) {
    ...
    try {
      await usersService.deleteUser(accessToken, userId, { confirm_password: deletePassword });
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 gap-4">
          {users.map((user) => {
            ...
            const showPassword = passwordVisibility[user.id];
                ...
                {isEditing ? (
                  ...
                  <div>
                    ...
                    <div className="pt-4 border-t-2 border-[var(--border-color)]">
                      <label className="block text-xs font-bold text-[var(--fg-subtle)] mb-2 uppercase tracking-wide">
                        {t("users.password")}
                      </label>
                      <div className="flex items-center gap-3">
                        <div className="flex-1 relative">
                          <input
                            ...
                            value={user.password || "••••••••"}
                            readOnly
                            className="w-full px-4 py-2.5 bg-[var(--bg-page)] border-2 border-[var(--border-color)] rounded-lg font-mono text-sm font-bold text-[var(--fg-heading)] cursor-default"
                          />
                        </div>
                        <button
                          onClick={() => togglePasswordVisibility(user.id)}
                          className="p-2.5 bg-[var(--bg-page)] border-2 border-[var(--border-color)] rounded-lg hover:border-[var(--color-oxygen-500)] hover:bg-[var(--bg-hover)] transition-all"
                          title={showPassword ? "Hide password" : "Show password"}
                        >
                          {showPassword ? (
                            <EyeOff size={18} className="text-[var(--fg-subtle)]" />
                          ) : (
                            <Eye size={18} className="text-[var(--fg-subtle)]" />
                          )}
                        </button>
                      </div>
                    </div>
